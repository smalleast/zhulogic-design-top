...
import { OSS_BASE_URL, ZHULOGIC_BASE_URL } from '@/constants'
...
export function windowOpen (uri) {
  const link = document.createElement('a')
  link.href = uri
  link.target = '_blank'
  document.body.appendChild(link)
...
export function styleBackgroundImage (key) {
  if (!key) return {}
...
export function goToDesignCloud (path) {
  window.location.href = `${ZHULOGIC_BASE_URL}${path}`
}
